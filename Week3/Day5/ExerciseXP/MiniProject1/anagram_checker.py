...
class AnagramChecker:
    def __init__(self, word_list_filename: str) -> None:
        with open(word_list_filename) as f:
            self.words: set[str] = set([
                word.strip().lower() for word in f.readlines()
            ])
    ...
    def get_anagrams(self, word: str) -> list[str]:
        # If dictionary is sufficiently large compared to words in it
        # Then checking all n! permutations is faster, than checking all pairs
        anagrams: list[str] = []
        word = word.lower()
        for letters in permutations(word, len(word)):
            anagram = "".join(letters)
            if anagram != word and self.is_valid_word(anagram):
